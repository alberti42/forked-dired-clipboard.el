...
On macOS, Finder exposes file URLs on the pasteboard but does not provide the
same general-purpose cut marker used by Windows Explorer or GNOME/MATE file
managers.  Dired-to-Dired ~C-w~ still moves files, while Finder interoperability
uses the standard file URL pasteboard behavior.
